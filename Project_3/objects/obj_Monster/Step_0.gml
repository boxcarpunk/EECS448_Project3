...
if(distance_to_object(obj_Player) < AggroRange)
{
	if(MoveOrDelay) //move after a delay
	{
		...
		{
			mp_potential_path(MonsterPatrol, obj_Player.x, obj_Player.y, MoveSpeed, 4, 0); //set a path toward the character
		...
		{
			obj_Player.Health -= MeleeDamage; //make the player take damage
		}
