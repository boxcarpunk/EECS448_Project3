...
if(global.pause){
	...
	draw_roundrect_color(camera_get_view_x(view_camera[0]) + profileWidth + healthOffset,camera_get_view_y(view_camera[0]) + profileWidth + healthOffset+100, camera_get_view_x(view_camera[0])+profileWidth + healthOffset,camera_get_view_y(view_camera[0])+profileWidth + healthOffset+100,c_black, c_black, false);
	/*draw_sprite(PlayerHealth,1,view_xport[0]+profileWidth + healthOffset,view_yport[0]+profileWidth + healthOffset);
	draw_sprite_ext(PlayerHealth,2,view_xport[0]+profileWidth + healthOffset,view_yport[0]+profileWidth + healthOffset,myHealth/ maxHealth, 1, 0,c_white,1);
	draw_sprite(PlayerHealth,0,view_xport[0]+profileWidth + healthOffset,view_yport[0]+profileWidth + healthOffset);
	draw_sprite_ext(AlexBardas,0,view_xport[0]+profileWidth + healthOffset-profileWidth+2,view_yport[0]+profileWidth + healthOffset - profileWidth/2 + profileWidth/2,size,size,0,c_white,1);
	draw_sprite(PlayerProfile,1,view_xport[0]+profileWidth + healthOffset-profileWidth+2,view_yport[0]+profileWidth + healthOffset - profileWidth/2 + profileWidth/2);
	*/
	//draw player name
	...
	ability1X = camera_get_view_x(view_camera[0]) - profileWidth/2 + 100;
	ability1Y = camera_get_view_y(view_camera[0]) + camera_get_view_height(view_camera[0])- profileWidth/2 - 10;
	//draw_sprite(PlayerProfile, 2, ability1X - profileWidth/2,ability1Y - profileWidth/2);
	...
	if(cooldown1TimeLeft > 0) {
		...
		draw_primitive_begin(pr_trianglefan);
			...
			draw_vertex(ability1X, ability1Y - profileWidth/2);
			
			//draw the corners of the cooldown box
			if(cooldown1TimeLeft > .125) {draw_vertex(ability1X - profileWidth/2, ability1Y - profileWidth/2);}
			if(cooldown1TimeLeft > .375) {draw_vertex(ability1X - profileWidth/2, ability1Y + profileWidth/2);}
			if(cooldown1TimeLeft > .625) {draw_vertex(ability1X + profileWidth/2, ability1Y + profileWidth/2);}
			if(cooldown1TimeLeft > .875) {draw_vertex(ability1X + profileWidth/2, ability1Y - profileWidth/2);}
			...
			if((degreeOfRotation + 360 + 90) % 180 > 45 && (degreeOfRotation + 360 + 90) % 180 < 135){
				edgeConst = abs(profileWidth/2/cooldownX);
			}
			else{
				edgeConst = abs(profileWidth/2/cooldownY);
			}
